		...
		<div class="mt-3">
			<table class="table table-bordered table-sm">
				...
				<tbody>
					<?php 
						$no  = 1; 
						$row = $sub_rincian = $tot_rincian = 0;
					?>
					@foreach($rekap AS $inventarisasi)
					<tr>
						<td colspan="8" class="small">
							<span class="ml-3">No. Jurnal: {{zerofy($inventarisasi->id, 5) }}</span>
							<span class="ml-3">Tgl. BA: {{datify($inventarisasi->tgl_ba)}}</span>
							<span class="ml-3">No. BA: {{$inventarisasi->no_ba}}</span>
							<span class="ml-3">Keterangan: {{$inventarisasi->keterangan}}</span>
						</td>
					</tr>
					<!-- ASET -->
					@foreach($inventarisasi->rincian->aset AS $aset)
					<tr class="small">
