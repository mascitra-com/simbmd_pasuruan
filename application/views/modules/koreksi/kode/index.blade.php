				...
				<tbody>
					...
					<tr class="small">
						...
						<td class="text-center">
							...
							<button class="btn btn-secondary btn-sm" id="btn-pesan">draf</button>
							@elseif($item->status_pengajuan === '1')
							<button class="btn btn-warning btn-sm" id="btn-pesan">menunggu</button>
							...
							<button class="btn btn-danger btn-sm" data-id-koreksi="{{$item->id}}"><i class="fa fa-comment-o mr-2"></i>ditolak</button>
							@else
							ERROR
							@endif
						</td>
...
<script>
	...
	$("[data-id-batal]").on('click', function(e){
		...
		$.getJSON("{{site_url('koreksi/kode/get_abort_status/')}}"+id, function(result){
			if (result.status === true) {
				html = "<div class='btn-group'>"
				html += "<a href='{{site_url()}}koreksi/kode/abort_transaction/"+id+"' class='btn btn-warning'>Batalkan Persetujuan</a>";
