...
<li class="breadcrumb-item"><a href="{{site_url('koreksi/nilai?id_organisasi='.$filter['id_organisasi'])}}">Koreksi</a></li>
<li class="breadcrumb-item active">Koreksi Nilai</li>
@end
				...
				<tbody>
					...
					<tr class="small">
						...
						<td class="text-center">
							...
							<button class="btn btn-sm btn-secondary btn-sm" id="btn-pesan">draf</button>
							@elseif($item->status_pengajuan === '1')
							<button class="btn btn-sm btn-warning btn-sm" id="btn-pesan">menunggu</button>
							...
							<button class="btn btn-sm btn-danger btn-sm" data-id-koreksi="{{$item->id}}"><i class="fa fa-comment-o mr-2"></i>ditolak</button>
							@else
							ERROR
							@endif
						</td>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">Tambah Koreksi Nilai</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			</div>
			<div class="modal-body">
				<form action="{{site_url('koreksi/nilai/insert')}}" class="form-row" method="POST">
...
<script>
	...
	$("[data-id]").on('click', function(){
		var id = $(this).data('id');
		$("#btn-hapus-confirm").attr("href", "{{site_url('koreksi/nilai/delete/')}}"+id);
	...
	$("[data-id-batal]").on('click', function(e){
		...
		$.getJSON("{{site_url('koreksi/nilai/get_abort_status/')}}"+id, function(result){
			if (result.status === true) {
				html = "<div class='btn-group'>"
				html += "<a href='{{site_url()}}koreksi/nilai/abort_transaction/"+id+"' class='btn btn-warning'>Batalkan Persetujuan</a>";
