...
<li class="breadcrumb-item active">Kibd</li>
@end
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{site_url('koreksi/atribut/kibd/insert')}}" method="POST">
...
<script>
	...
	$("table > tbody").delegate('[data-id]', 'click', function(e){
		...
		$.getJSON("{{site_url('koreksi/atribut/kibd/get/'.$koreksi->id.'?id=')}}"+id, function(result){
			$("[name=id]").val(result.rows[0].id);
			$("[name=luas]").val(result.rows[0].luas);
			$("[name=panjang]").val(result.rows[0].panjang);
			$("[name=lebar]").val(result.rows[0].lebar);
			$("[name=lokasi]").val(result.rows[0].lokasi);
			$("[name=kontruksi]").val(result.rows[0].kontruksi);
