...
<li class="breadcrumb-item active">Kibe</li>
@end

@section('content')
<div class="card">
	<div class="card-header">KIB-C (Buku, Barang & Kebudayaan)</div>
	<div class="card-body">
		<table class="jq-table table-striped" data-search="true" data-show-columns="true" data-search-on-enter-key="true" data-pagination="true" data-side-pagination="server" data-url="{{site_url('koreksi/atribut/kibe/get/'.$koreksi->id)}}">
			<thead>
				<tr>
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{site_url('koreksi/atribut/kibe/insert')}}" method="POST">
					...
					<div class="row">
						...
						<div class="form-group col">
							...
							<input type="text" class="form-control" name="pencipta" placeholder="Pencipta">
						</div>
						<div class="form-group col">
...
<script>
	...
	$("table > tbody").delegate('[data-id]', 'click', function(e){
		...
		$.getJSON("{{site_url('koreksi/atribut/kibe/get/'.$koreksi->id.'?id=')}}"+id, function(result){
			$("[name=id]").val(result.rows[0].id);
			$("[name=judul]").val(result.rows[0].judul);
			$("[name=pencipta]").val(result.rows[0].pencipta);
