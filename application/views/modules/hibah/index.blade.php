				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
							...
							<div class="btn-group">
								<button class="btn btn-success btn-sm btn-block" data-id-hibah="{{$item->id}}"><i class="fa fa-comment-o mr-2"></i>disetujui</button>
							...
							<button class="btn btn-danger btn-sm btn-block" data-id-hibah="{{$item->id}}"><i class="fa fa-comment-o mr-2"></i>ditolak</button>
							@else
							ERROR
							@endif
						</td>
...
<script>
	...
	$("[data-id-hibah]").on('click', function(e){
		var id = $(e.currentTarget).data('id-hibah');
	...
	$("[data-id-batal]").on('click', function(e){
        ...
        $.getJSON("{{site_url('hibah/index/get_abort_status/')}}"+id, function(result){
            if (result.status === true) {
                html = "<div class='btn-group'>"
                html += "<a href='{{site_url()}}hibah/index/abort_transaction/"+id+"' class='btn btn-warning'>Batalkan Persetujuan</a>";
