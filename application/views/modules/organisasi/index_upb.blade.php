...
<li class="breadcrumb-item"><a href="{{site_url('organisasi/subunit/'.$induk->sub_dari)}}">Sub-Unit</a></li>
<li class="breadcrumb-item active">UPB</li>
@end
		...
		<div class="card">
			<div class="card-header form-inline">
				...
				<div class="btn-group">
					...
					<a href="{{site_url('organisasi/add/4?id='.$induk->id)}}" class="btn btn-primary btn-tambah"><i class="fa fa-plus mr-2"></i>Tambah</a>
				</div>
			</div>
				...
				<table class="table table-striped table-hover table-bordered table-sm">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{zerofy($org->kd_subunit)}}</td>
							<td>{{zerofy($org->kd_upb)}}</td>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="{{site_url('organisasi/lebur')}}" method="POST">
					...
					<input type="hidden" name="ref" value="{{'organisasi/upb/'.$induk->id}}">
...
<script type="text/javascript">
	var organisasi = (function(){
		...
		$("[data-id]").on('click', function(e){
			...
			$.getJSON("{{site_url('organisasi/get?jenis=4&sort=nama&order=asc&sub_dari='.$induk->id)}}", function(result){
