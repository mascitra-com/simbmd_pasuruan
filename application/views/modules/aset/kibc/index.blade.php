...
@section('title'){{$is_kdp?'KDP-C':'KIB-C'}}@end

@section('breadcrump')
<li class="breadcrumb-item"><a href="{{site_url()}}">Beranda</a></li>
<li class="breadcrumb-item">Aset Saldo {{$source==='saldo'?'Awal':$source}}</li>
<li class="breadcrumb-item active">{{$is_kdp?'KDP-C':'KIB-C'}}</li>
@end
...
<div class="card">
	<div class="card-header form-inline">
		<span>{{$is_kdp?'KDP-C':'KIB-C'}} (Gedung &amp Bangunan)</span>
		<form action="{{site_url('aset/kibc')}}" method="GET" class="ml-auto">
	...
	<div class="card-body">
		<table class="jq-table table-striped" data-search="true" data-show-columns="true" data-search-on-enter-key="true" data-pagination="true" data-side-pagination="server" data-url="{{site_url('aset/kibc/get/'.$id_organisasi.'/'.$source)}}{{$is_kdp?'?is_kdp=true':''}}">
			<thead>
					<tr>
...
<script type="text/javascript">
	var kib = (function(){
		...
		// GET RINCIAN
		$.getJSON("{{site_url('aset/kibc/get_rincian_widget/'.$id_organisasi.'/'.$is_kdp.'/'.$source)}}", function(result){
