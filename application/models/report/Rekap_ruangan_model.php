...
class Rekap_ruangan_model extends MY_Model {
	...
	{
        $select = 'sast.id, sast.kd_pemilik, o.kd_bidang, o.kd_unit, kd_subunit, kd_upb, sast.tgl_perolehan, sast.reg_barang, sast.tipe, sast.merk, sast.ukuran, sast.id_ruangan, sast.keterangan, kd_golongan,kd_bidang,kd_kelompok,kd_subkelompok,kd_subsubkelompok,nama,
        ...
        $select2 = 'ast.id, ast.kd_pemilik, o.kd_bidang, o.kd_unit, kd_subunit, kd_upb, ast.tgl_perolehan, ast.reg_barang, ast.tipe, ast.merk, ast.ukuran, ast.id_ruangan, ast.keterangan, kd_golongan,kd_bidang,kd_kelompok,kd_subkelompok,kd_subsubkelompok,nama,
		SUM(CASE WHEN (kondisi=1) THEN 1 ELSE 0 END) AS kb, SUM(CASE WHEN (kondisi=2) THEN 1 ELSE 0 END) AS kkb, SUM(CASE WHEN (kondisi=3) THEN 1 ELSE 0 END) AS krb,
