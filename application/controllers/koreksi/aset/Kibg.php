...
class Kibg extends MY_Controller
{
    ...
    public function koreksi_nilai($id_koreksi = NULL)
    {
        ...
        $data['pagination']     = $this->pagination->get_pagination($result['data_count'], $filter, 'koreksi/aset/kibg/koreksi_nilai/'.$id_koreksi);

        $this->render('modules/koreksi/nilai/kibg', $data);
    }
    ...
    public function insert_nilai()
    {
        ...
        if($sukses) {
            ...
            if($sukses2) {
                ...
                $this->go('koreksi/aset/kibg/koreksi_nilai/'.$id_koreksi);
            } else {
                # ROLL OUT
    ...
    public function koreksi_kepemilikan($id_koreksi = NULL)
    {
        ...
        # PENTING!!!
        // $where_not_in = $this->kib_temp->select('id_aset')->join('koreksi','temp_aset_g.id_koreksi=koreksi.id')->or_where(array('id_koreksi'=>$id_koreksi, 'status_pengajuan<>'=>2))->as_array()->get_all();
        $where_not_in = $this->kib_temp->select('id_aset')->join('koreksi','temp_aset_g.id_koreksi=koreksi.id')->where(array('id_koreksi'=>$id_koreksi))->as_array()->get_all();
        $where_not_in = array_column($where_not_in, 'id_aset');
        ...
        $result = $this->kib->where_not_in('aset_g.id', !empty($where_not_in)?$where_not_in:'')->get_data($filter);

        $data['filter']         = $filter;
        $data['kib']            = $result['data'];
        $data['terpilih_count'] = count($where_not_in);
        $data['organisasi']     = $this->organisasi->get_many_by(array('id<>'=>$data['koreksi']->id_organisasi->id, 'jenis'=>4));
        $data['pagination']     = $this->pagination->get_pagination($result['data_count'], $filter, 'koreksi/aset/kibg/koreksi_kepemilikan/'.$id_koreksi);

        $this->render('modules/koreksi/kepemilikan/kibg', $data);
    }
    ...
    public function insert_kepemilikan()
    {
        ...
        if($sukses) {
            ...
            if($sukses2) {
                $this->message('Data berhasil ditambah','success');
                $this->go('koreksi/aset/kibg/koreksi_kepemilikan/'.$id_koreksi);
            } else {
                # ROLL OUT
    ...
    public function koreksi_kode($id_koreksi = NULL)
    {
        ...
        $data['pagination']     = $this->pagination->get_pagination($result['data_count'], $filter, 'koreksi/aset/kibg/koreksi_kode/'.$id_koreksi);

        $this->render('modules/koreksi/kode/kibg', $data);
    }
    ...
    public function insert_kode()
    {
        ...
        if($sukses) {
            ...
            if($sukses2) {
                $this->message('Data berhasil ditambah','success');
                $this->go('koreksi/aset/kibg/koreksi_kode/'.$id_koreksi);
            } else {
                # ROLL OUT
                $this->koreksi_detail->delete($sukses);
    ...
    public function koreksi_hapus($id_koreksi = NULL)
    {
        ...
        $data['kib']            = $result['data'];
        $data['terpilih_count'] = count($where_not_in);
        $data['pagination']     = $this->pagination->get_pagination($result['data_count'], $filter, 'koreksi/aset/kibg/koreksi_hapus/'.$id_koreksi);

        $this->render('modules/koreksi/hapus/kibg', $data);
    }
    ...
    public function insert_hapus()
    {
        ...
        if($sukses) {
            $this->message('Data berhasil ditambah','success');
            $this->go('koreksi/aset/kibg/koreksi_hapus/'.$id_koreksi);
        } else {
                # ROLL OUT
            $this->koreksi_detail->delete($sukses);
