...
class Kibd extends MY_Controller
{
	...
	public function index($id_koreksi = null)
	{
		...
		$this->render('modules/koreksi/atribut/kibd', $data);
	}
	...
	{
		$kolom = array('tgl_perolehan', 'tgl_pembukuan', 'kontruksi', 'panjang', 'lebar', 'luas', 'lokasi', 'dokumen_tgl', 'dokumen_no', 'status_tanah', 'kode_tanah', 'asal_usul', 'kondisi', 'nilai');
	...
	private function finalize_data($data, $is_rincian = FALSE)
	{
		foreach ($data['rows'] as $key => $value) {
			...
			if (!$is_rincian) {
				...
				$value['aksi'] = "<a href='".site_url('koreksi/atribut/kibd/delete/'.$value['id'])."' class='btn btn-sm btn-danger btn-block'><i class='fa fa-times'></i></a>";
			}
	...
	public function insert()
	{
		...
		$this->go('koreksi/atribut/kibd/index/'.$id_koreksi);
	}
