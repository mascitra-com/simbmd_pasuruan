...
class Kibb extends MY_Controller
{
	...
	public function index($id_koreksi = null)
	{
		...
		$this->render('modules/koreksi/atribut/kibb', $data);
	}
	...
	{
		$kolom = array('tgl_perolehan', 'tgl_pembukuan', 'merk', 'tipe', 'ukuran', 'bahan', 'no_pabrik', 'no_rangka', 'no_mesin', 'no_polisi', 'no_bpkb', 'asal_usul', 'kondisi', 'nilai');
	...
	private function finalize_data($data, $is_rincian = FALSE)
	{
		foreach ($data['rows'] as $key => $value) {
			...
			if (!$is_rincian) {
				...
				$value['aksi'] = "<a href='".site_url('koreksi/atribut/kibb/delete/'.$value['id'])."' class='btn btn-sm btn-danger btn-block'><i class='fa fa-times'></i></a>";
			}
	...
	public function insert()
	{
		...
		$this->go('koreksi/atribut/kibb/index/'.$id_koreksi);
	}
