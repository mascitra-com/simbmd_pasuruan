...
class Kiba extends MY_Controller
{
	...
	public function index($id_koreksi = null)
	{
		...
		$this->render('modules/koreksi/atribut/kiba', $data);
	}
	...
	{
		$kolom = array('reg_barang','reg_induk','luas','alamat','sertifikat_tgl','sertifikat_no','hak','pengguna','tgl_perolehan','tgl_pembukuan','asal_usul','kondisi','nilai','keterangan');
	...
	private function finalize_data($data, $is_rincian = FALSE)
	{
		foreach ($data['rows'] as $key => $value) {
			...
			if (!$is_rincian) {
				...
				$value['aksi'] = "<a href='".site_url('koreksi/atribut/kiba/delete/'.$value['id'])."' class='btn btn-sm btn-danger btn-block'><i class='fa fa-times'></i></a>";
			}
	...
	public function insert()
	{
		...
		$this->go('koreksi/atribut/kiba/index/'.$id_koreksi);
	}
