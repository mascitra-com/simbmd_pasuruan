...
class Kibc extends MY_Controller
{
	...
	public function index($id_koreksi = null)
	{
		...
		$this->render('modules/koreksi/atribut/kibc', $data);
	}
		...
		foreach ($data['rows'] as $key => $value) {
			...
			foreach ($value as $index => $item) {
				switch ($index) {
					...
					if ($is_rincian && $item !== $corrected[$index]) {
						$value[$index] = empty($item) && $item!=='0'?'':"<b class='text-danger'>-(".$value[$index].")</b>";
						$value[$index] .= empty($corrected[$index]) && $corrected[$index]!=='0'?'':"<b class='text-success'> +(".($corrected[$index]==='1'?'iya':'tidak').")</b>";
					}
					break;
					default:
			...
			if (!$is_rincian) {
				...
				$value['aksi'] = "<a href='".site_url('koreksi/atribut/kibc/delete/'.$value['id'])."' class='btn btn-sm btn-danger btn-block'><i class='fa fa-times'></i></a>";
			}
	...
	public function insert()
	{
		...
		$this->go('koreksi/atribut/kibc/index/'.$id_koreksi);
	}
