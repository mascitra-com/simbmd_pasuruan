...
class Kode extends MY_Controller {
    ...
    public function rincian($id = NULL)
    {
        ...
        $this->render('modules/koreksi/kode/rincian', $data);
    }
    ...
    public function rincian_redirect()
    {
        ...
        switch ($jenis) {
            case 'a':
            $this->go('koreksi/aset/kiba/koreksi_kode/'.$id);
            break;
            case 'b':
            $this->go('koreksi/aset/kibb/koreksi_kode/'.$id);
            break;
            case 'c':
            $this->go('koreksi/aset/kibc/koreksi_kode/'.$id);
            break;
            case 'd':
            $this->go('koreksi/aset/kibd/koreksi_kode/'.$id);
            break;
            case 'e':
            $this->go('koreksi/aset/kibe/koreksi_kode/'.$id);
            break;
            case 'g':
            $this->go('koreksi/aset/kibg/koreksi_kode/'.$id);
            break;
    ...
    public function insert()
    {
        ...
        if (!$this->koreksi->form_verify($data)) {
            ...
            $this->go('koreksi/kode?id_organisasi='.$data['id_organisasi']);
        }
        ...
        if($sukses) {
            ...
            $this->go('koreksi/kode/rincian/'.$sukses);
        } else {
    ...
    public function update()
    {
        ...
        if (!$this->koreksi->form_verify($data)) {
            $this->message('Isi data yang perlu diisi.', 'warning');
            $this->go('koreksi/kode/rincian/'.$id);
        }
    ...
    public function delete($id_koreksi = null)
    {
        ...
        if ($koreksi->status_pengajuan == 1 OR $koreksi->status_pengajuan == 2) {
            ...
            $this->go('koreksi/kode?id_organisasi='.$koreksi->id_organisasi);
            die();
        }
    ...
    private function subtitute($data)
    {
    	...
    	foreach ($data AS $kib) {
    		foreach ($kib as $item) {
    			$item->corrected_value = $this->kategori->get($item->corrected_value);
    		}
    	}
    	return $data;
    }
    ...
    public function abort_transaction($id_koreksi = NULL)
    {
        # JIKA KOSONG
        if (empty($id_koreksi)) {
            $this->message('Pilih data koreksi kode yang akan dibatalkan', 'danger');
            $this->go('koreksi/kode/');
        }
        ...
        foreach ($alfabet as $item) {
            ...
            foreach ($temp as $aset) {
                ...
                $this->{$model_kib}->update($aset->id_aset, array('id_kategori'=>$orginal_value));
            }
        }
