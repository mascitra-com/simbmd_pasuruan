...
class Koreksi_kode extends MY_Controller {
	...
	public function index() {
		...
		$filter['jenis_koreksi'] 	= '3';
		$filter['ord_by']  = 'log_time';
		$filter['ord_pos'] = 'DESC';
		$result = $this->koreksi->get_data_persetujuan($filter);

		$data['koreksi'] 	= $result['data'];
		$data['pagination'] = $this->pagination->get_pagination($result['data_count'], $filter, 'persetujuan/koreksi_kode');
		$data['filter'] 	= $filter;

		$this->render('modules/persetujuan/koreksi/kode/index', $data);
	}
	...
	public function rincian($id = NULL)
    {
        ...
        $this->render('modules/persetujuan/koreksi/kode/rincian', $data);
    }
    ...
    public function verifikasi() 
	{
		...
		if ($sukses) {
			...
			if ($sukses2) {
				...
				$this->go('persetujuan/koreksi_kode');
			} else {
				# ROLL OUT
	...
	private function koreksi($id_koreksi)
	{
		...
		foreach ($alfabet as $item) {
			...
			foreach ($temp as $kib) {
				...
				->update("aset_{$item}", array('id_kategori'=>$kib->corrected_value));
			}
		}
		return 1;
	}
