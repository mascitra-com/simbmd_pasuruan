...
class Kib_non extends MY_Controller {
	...
	{
		parent::__construct();
		$this->load->model('aset/inventarisasi/Kibnon_model', 'kib');
	...
	public function index()
	{
		...
		$this->render('modules/aset/inventarisasi/kibnon/index', $data);
	}
	...
	public function add($id = null)
	{
		if(empty($id)) {
			...
			$this->go('aset/inventarisasi/kib_non');
		}

		$data['org'] = $this->organisasi->get($id);
		$this->render('modules/aset/inventarisasi/kibnon/form', $data);
	}
	...
	public function insert()
	{
		...
		if (!$this->kib->form_verify($data)) {
            ...
            $this->go('aset/inventarisasi/kib_non/add/'.$data['id_organisasi']);
        }
	...
	public function update()
	{
		...
		if (!$this->kib->form_verify($data)) {
			$this->message('Isi data yang wajib diisi', 'danger');
            $this->go('aset/inventarisasi/kib_non/edit/'.$id);
        }

		$sukses = $this->kib->update($id, $data);
		if($sukses) {
			$this->message('Data berhasil disunting','success');
			$this->go('aset/inventarisasi/kib_non?id_organisasi='.$data['id_organisasi']);
		} else {
	...
	public function delete($id = NULL)
	{
		...
		if($sukses) {
			$this->message("Data berhasil dihapus, <a href='".site_url('aset/inventarisasi/kib_non/undelete/'.$id)."'><b>Urungkan!</b></a>",'success');
			$this->go('aset/inventarisasi/kib_non');
		} else {
